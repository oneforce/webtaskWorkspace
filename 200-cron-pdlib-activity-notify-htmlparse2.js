...
module.exports = function (context, req, res) {
  ...
  http.get('http://www.pdlib.com/pdtsg_website/html/defaultsite/pd_tsg_tsfw_setd/List/list_0.htm',function(req,res){
      ...
      req.on('end',function(){
        var document = htmlparser2.parseDOM(html);
        // console.log(document[2]); // html
        // console.log(document[2].children[3]);//body
        var ulArrays = document[2].children[3].children[11].children[3].children;
        for( var i in ulArrays) {
          var ul_node = ulArrays[i];
          // console.log("ul -> " + ul_node.name);
          if(ul_node.name == 'ul'){
            for(var j in ul_node.children){
              var ul_node_j = ul_node.children[j];
              //console.log("   " + ul_node_j.nodeName + " : " + ul_node_j.value);
              if(ul_node_j.name == 'li') {
                var li_node = ul_node_j;
                var textDate = ul_node_j.children[0].children[0].data;
                var title = ul_node_j.children[1].attribs.title;
                var link = 'http://www.pdlib.com/pdtsg_website/html/defaultsite/' + ul_node_j.children[1].attribs.href.substr(6);
